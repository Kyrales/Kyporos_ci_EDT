...
Функция ПолучитьПоследнююВерсиюИзменений(ОбъектМетаданных, ТекущаяВерсия, ТипМодуля) Экспорт 
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураКлюча = Новый Структура("Конфигурация, ВерсияКонфигурации, ОбъектМетаданных, ТипМодуля");
...
Процедура ЗаполнитьНовуюВерсиюКонфигурации(Знач НоваяВерсия, Знач ПоследнейВерсиейИзмененияДляВсех = Ложь) Экспорт
	...
	НаборЗаписей = РегистрыСведений.УчетВерсийКонфигурации.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Конфигурация.Установить(НоваяВерсия.Владелец);
	...
	Запрос.УстановитьПараметр("Конфигурация", НоваяВерсия.Владелец);
	...
	Пока Выборка.Следующий() Цикл
		...
		МенеджерЗаписиУчетаВерсииКонфигурации = РегистрыСведений.УчетВерсийКонфигурации.СоздатьМенеджерЗаписи();
	...
	Если ПоследнейВерсиейИзмененияДляВсех Тогда
		 ...
		 ЗаполнитьПоследнейВерсиейИзмененияДляВсехОМ(НоваяВерсия.Владелец, НоваяВерсия);
