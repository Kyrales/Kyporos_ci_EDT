...
//  + [Процедура]ПроверитьНомерСправочникаПоОрганизации - выполняет проверку модифицированности реквизита Организация элемента содержащего номер
//  * [Процедура]ПриОпределенииПрефиксаОрганизации - в случае если проект, до получение напрямую из него префикса
//  * [Процедура]УстановитьПрефикс - (НЕ ИСПОЛЬЗУЕТСЯ, т.к. есть проблемы в функционировании) возможность получения префикса рекурсивно. Например: Проект.Организация - получение и у проекта и организации последовательно.
// 
// ТЕГИ ИЗМЕНЕНИЙ
//
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
Процедура ПриОпределенииПрефиксаОрганизации(Знач Организация, ПрефиксОрганизации) Экспорт
	...
	// { *Олег Каратаев [03.02.2018]
	Если ТипЗнч(Организация) = Тип("СправочникСсылка.Проекты") Тогда
...
Процедура УстановитьПрефикс(Источник, Префикс, УстановитьПрефиксИнформационнойБазы, УстановитьПрефиксОрганизации)
	...
	Если УстановитьПрефиксОрганизации Тогда
		...
		Если РеквизитОрганизацияДоступен(Источник) Тогда
			...
			// { *Олег Каратаев [03.02.2018]
			// *НЕ ИСПОЛЬЗУЕТСЯ, т.к. есть обращения в других местах
			// Определение источника, в случае если в имя реквизита переопределяемого модуля передан вариант через точку, типа: Проект.Организация.
			// Система проверит каждый реквизит на предмет наличия заполненного поля префикс. Первый у кого он будет заполнен, тот реквизит и будет использован.
			// Например для варианта "Проект.Организация": 
			// 1. Проект содержит заполненный префикс. При формировании номера будет использован префикс проекта.
			// 2. Проект не содержит заполненный префикс, а организация проекта содержит заполненный префикс. При формировании номера будет использован префикс организации проекта.
			
			ИмяРеквизитаОрганизация = ИмяРеквизитаОрганизация(Источник.Метаданные());
			Если СтрНайти(ИмяРеквизитаОрганизация,".")>0 Тогда
				
				МассивРеквизитовДляПрефикса = СтрРазделить(ИмяРеквизитаОрганизация,".");
				
				РеквизитИсточника = Источник[МассивРеквизитовДляПрефикса[0]];
				ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(МассивРеквизитовДляПрефикса,МассивРеквизитовДляПрефикса[0]);
				
				ПрефиксОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитИсточника,"Префикс");
				ИсточникОрганизация = РеквизитИсточника;
				
				Если Не ЗначениеЗаполнено(ПрефиксОрганизации) Тогда
					
					Для каждого ТекРеквизитДляПрефикса Из МассивРеквизитовДляПрефикса Цикл
						
						РеквизитыДочернейСсылки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитИсточника,ТекРеквизитДляПрефикса+","+ТекРеквизитДляПрефикса+".Префикс");
						
						РеквизитИсточника =  РеквизитыДочернейСсылки[ТекРеквизитДляПрефикса];
						
						Если ЗначениеЗаполнено(РеквизитыДочернейСсылки[ТекРеквизитДляПрефикса+"Префикс"]) Тогда
							
							ИсточникОрганизация = РеквизитИсточника;
							ПрефиксОрганизации = РеквизитыДочернейСсылки[ТекРеквизитДляПрефикса+"Префикс"];
							Прервать;
				...
				КонецЕсли;  // Не ЗначениеЗаполнено(ПрефиксРеквизитаИсточника)
				
			Иначе
				
				ИсточникОрганизация = Источник[ИмяРеквизитаОрганизация];			
				
				ПриОпределенииПрефиксаОрганизации(
					ИсточникОрганизация, ПрефиксОрганизации);
...
// Выполняет проверку модифицированности реквизита Организация элемента справочника.
// Если реквизит Организация изменен, то Номер элемента обнуляется.
// Это необходимо для назначения нового номера элементу.
//
// Параметры:
//  Источник - СправочникОбъект - источник события подписки.
//  Отказ    - Булево - флаг отказа.
// 
Процедура ПроверитьНомерСправочникаПоОрганизации(Источник, Отказ) Экспорт
