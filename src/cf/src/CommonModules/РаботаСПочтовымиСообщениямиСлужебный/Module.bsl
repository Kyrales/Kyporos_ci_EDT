...
//  * [Функция]АдаптированныйНаборПисем #ВходящиеЭлектронныеПисьма - добавления параметра ПочтовоеСообщение к таблице писем, Исключение дублей по имени
//  * [Процедура]ЗаполнитьВложенияПоПисьму #ВходящиеЭлектронныеПисьма - Помещаем только EML во вложение
// 
// ТЕГИ ИЗМЕНЕНИЙ
//
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
Функция АдаптированныйНаборПисем(Знач НаборПисем, Знач Колонки = Неопределено)
	...
	Для Каждого ПочтовоеСообщение Из НаборПисем Цикл
		...
		Для Каждого НаименованиеКолонки Из Колонки Цикл
			// { *Олег Каратаев [12.02.2017]
			Если НаименованиеКолонки = "ПочтовоеСообщение" Тогда
				НоваяСтрока[НаименованиеКолонки] = ПочтовоеСообщение;
				Продолжить
			КонецЕсли;
			// }
			...
			ИначеЕсли ТипЗнч(ПолеПисьма) = Тип("ИнтернетПочтовыеВложения") Тогда
				...
				Для Каждого Вложение Из ПолеПисьма Цикл
					Если ТипЗнч(Вложение.Данные) = Тип("ДвоичныеДанные") Тогда
						// { *Олег Каратаев [12.01.2020]
						// Исключение дублей по имени.
						Если Вложения.Получить(Вложение.Имя) <> Неопределено Тогда
							// имя дублируется, а значит нужно создать новое имя файла
							РасширениеИмениФайла = ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(Вложение.Имя);
							ИмяФайлаПолноеНовое = ПолучитьИмяВременногоФайла(РасширениеИмениФайла);
							ИмяФайлаНовое = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ИмяФайлаПолноеНовое).Имя;
							Вложения.Вставить(ИмяФайлаНовое, Вложение.Данные);
						Иначе
...
Процедура ЗаполнитьВложенияПоПисьму(Вложения, Письмо)
	...
	// { *Олег Каратаев [12.01.2020]
	// Помещаем только EML во вложение.
	ПредставлениеПисьма = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ПредставлениеПисьма, "_");
	ВложениеEML = Письмо.ПолучитьИсходныеДанные();
	ИмяEML = ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(ПредставлениеПисьма, "eml");
	Вложения.Вставить(ИмяEML, ВложениеEML);
	//---1C---
	//Индекс = 0;
