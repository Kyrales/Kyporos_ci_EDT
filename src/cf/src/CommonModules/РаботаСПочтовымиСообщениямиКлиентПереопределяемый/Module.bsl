...
//  * [Процедура]ПередОткрытиемФормыОтправкиПисьма #ВходящиеЭлектронныеПисьма
// 
// ТЕГИ ИЗМЕНЕНИЙ
//
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
Процедура ПередОткрытиемФормыОтправкиПисьма(ПараметрыОтправки, ОбработчикЗавершения, СтандартнаяОбработка) Экспорт
	...
	// { *Олег Каратаев [21.12.2019]
	
	Если ТипЗнч(ПараметрыОтправки.Предмет) = Тип("ЗадачаСсылка.ЗадачиПользователя") Тогда
		
		РаботаСЗадачамиВызовСервера.ПередОткрытиемФормыОтправкиПисьмаЗадачи(ПараметрыОтправки.Предмет, ПараметрыОтправки);
