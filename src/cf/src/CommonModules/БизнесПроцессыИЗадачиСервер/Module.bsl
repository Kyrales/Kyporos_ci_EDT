...
//  * [Процедура]ЗаблокироватьЗадачи - настройка блокировки объекта "ЗадачиПользователя"  #TaskUsers
//  * [Функция]ВыбратьНовыеЗадачиПоИсполнителям - использование объекта метаданных ЗадачиПользователя вместо ЗадачаИсполнителя  #TaskUsers
//  * [Функция]ОтправитьУведомлениеОНовыхЗадачах - разделитель писем #TaskUsers
//  * [Процедура]УведомитьИсполнителейОНовыхЗадачах - разделение от службы поддержки #TaskUsers
//  * [Процедура]УстановитьОформлениеЗадач - не выделять красным задачи у которых состояние без признака УчитыватьПриРасчетеВремениВыполнения #TaskUsers
//  + [Процедура]УведомлениеСлужбыТехподдержкиОНовыхЗадачах - уведомление только для исполнителей с флагом "Служба техподдержки" #TaskUsers
//  + [Функция]СформироватьПредставлениеЗадачиПользователя - описание выводить в письме полное #TaskUsers
//
// ТЕГИ ИЗМЕНЕНИЙ
//  #TaskUsers
//   - Типы писем по задачам переведены в тип HTML для повышения юзабилити
//   - Функция уведомления службы поддержки о задачах использует расширенный анализ выборки задач для возможности уведомления при любом изменении их (только для исполнителей с флагом "Служба техподдержки"). Вызывается из регламентного задания "Уведомление службы техподдержки о новых задачах" 
//
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
Процедура УстановитьОформлениеЗадач(Знач СписокЗадачИлиЕгоУсловноеОформление) Экспорт
	...
	// { *Олег Каратаев [15.11.2019]
	ЭлементОтбораДанных = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбораДанных.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Состояние.УчитыватьПриРасчетеВремениВыполнения");
	ЭлементОтбораДанных.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
Процедура ЗаблокироватьЗадачи(Задачи) Экспорт
	...
	Если ТипЗнч(Задачи) = Тип("Массив") Тогда
		Для каждого Задача Из Задачи Цикл
			...
			// { *Олег Каратаев [16.02.2014]
			Если ТипЗнч(Задача) = Тип("ЗадачаСсылка.ЗадачиПользователя") Тогда
	...
	Иначе	
		...
		КонецЕсли;	
		// { *Олег Каратаев [16.02.2014]
		Если ТипЗнч(Задачи) = Тип("ЗадачаСсылка.ЗадачиПользователя") Тогда
			ЭлементБлокировки = Блокировка.Добавить("Задача.ЗадачиПользователя");
		Иначе
			ЭлементБлокировки = Блокировка.Добавить("Задача.ЗадачаИсполнителя");
		КонецЕсли;
...
Процедура УведомитьИсполнителейОНовыхЗадачах() Экспорт
	...
	Для Каждого СтрокаИсполнителя Из ЗадачиПоИсполнителям.Строки Цикл
		// { *Олег Каратаев [16.09.2014]
		Если СтрокаИсполнителя.Исполнитель.СлужбаТехподдержки Тогда
			Продолжить;	
		КонецЕсли;
		// }
...
Функция ВыбратьНовыеЗадачиПоИсполнителям(Знач ДатаВремяОт, Знач ДатаВремяПо)
	...
	// { *Олег Каратаев [16.07.2013]
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ИсторияСобытийЗадач.Задача КАК Задача,
	               |	МАКСИМУМ(ИсторияСобытийЗадач.ДатаСобытия) КАК ДатаСобытия,
	               |	ИсторияСобытийЗадач.Событие КАК Событие
	               |ПОМЕСТИТЬ ВТ_ПоследниеСобытияЗадач
	               |ИЗ
	               |	РегистрСведений.ИсторияСобытийЗадач КАК ИсторияСобытийЗадач
	               |ГДЕ
	               |	ИсторияСобытийЗадач.ДатаСобытия МЕЖДУ &ДатаВремяОт И &ДатаВремяПо
	               |	И ИсторияСобытийЗадач.Событие В(&СписокСобытий)
	               |	И НЕ ИсторияСобытийЗадач.Задача.ПометкаУдаления
	               |	И ИсторияСобытийЗадач.Задача.УзелОбмена = &УзелОбмена
	               |
	               |СГРУППИРОВАТЬ ПО
	               ...
	               |	ИсторияСобытийЗадач.Пользователь КАК Пользователь,
	               |	ИсторияСобытийЗадач.Событие КАК Событие,
	               |	ИсторияСобытийЗадач.Комментарий КАК Комментарий
	               |ПОМЕСТИТЬ ВТ_СобытияЗадачПолные
	               |ИЗ
	               |	РегистрСведений.ИсторияСобытийЗадач КАК ИсторияСобытийЗадач
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ПоследниеСобытияЗадач КАК ВТ_ПоследниеСобытияЗадач
	               |		ПО ИсторияСобытийЗадач.Задача = ВТ_ПоследниеСобытияЗадач.Задача
	               |			И ИсторияСобытийЗадач.ДатаСобытия = ВТ_ПоследниеСобытияЗадач.ДатаСобытия
	               |			И ИсторияСобытийЗадач.Событие = ВТ_ПоследниеСобытияЗадач.Событие
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	               |	ВложенныйЗапрос.Задача КАК Задача,
	               |	МАКСИМУМ(ВложенныйЗапрос.Пользователь) КАК ПользовательРедактировавшийЗадачу,
	               |	МАКСИМУМ(ВложенныйЗапрос.СобытиеОписание) КАК СобытиеОписание,
	               |	МАКСИМУМ(ВложенныйЗапрос.СобытиеПеренаправление) КАК СобытиеПеренаправление,
	               |	МАКСИМУМ(ВложенныйЗапрос.СобытиеСрок) КАК СобытиеСрок,
	               |	МАКСИМУМ(ВложенныйЗапрос.СобытиеВажностьИПрочее) КАК СобытиеВажностьИПрочее,
	               |	МАКСИМУМ(ВложенныйЗапрос.СобытиеВозврат) КАК СобытиеВозврат,
	               |	МАКСИМУМ(ВложенныйЗапрос.СобытиеПрочиеИзменения) КАК СобытиеПрочиеИзменения
	               |ПОМЕСТИТЬ ИсторийСобытийИзменений
	               |ИЗ
	               |	(ВЫБРАТЬ
	               |		ВТ_СобытияЗадачПолные.Задача КАК Задача,
	               |		ВТ_СобытияЗадачПолные.Пользователь КАК Пользователь,
	               |		""#"" + ""Дополнение описания"" КАК СобытиеОписание,
	               |		"""" КАК СобытиеВажностьИПрочее,
	               |		"""" КАК СобытиеПеренаправление,
	               |		"""" КАК СобытиеСрок,
	               |		"""" КАК СобытиеВозврат,
	               |		"""" КАК СобытиеПрочиеИзменения
	               |	ИЗ
	               |		ВТ_СобытияЗадачПолные КАК ВТ_СобытияЗадачПолные
	               |	ГДЕ
	               |		ВТ_СобытияЗадачПолные.Событие = ЗНАЧЕНИЕ(Перечисление.ВидыСобытийЗадач.ДополнениеОписания)
	               |		И ВТ_СобытияЗадачПолные.ДатаСобытия МЕЖДУ &ДатаВремяОт И &ДатаВремяПо
	               |	
	               |	ОБЪЕДИНИТЬ ВСЕ
	               |	
	               |	ВЫБРАТЬ
	               |		ВТ_СобытияЗадачПолные.Задача,
	               |		ВТ_СобытияЗадачПолные.Пользователь,
	               |		"""",
	               |		""#"" + ""Изменена важность: "" + ПОДСТРОКА(ВТ_СобытияЗадачПолные.Комментарий, 1, 150) + "" "",
	               |		"""",
	               |		"""",
	               |		"""",
	               |		""""
	               ...
	               |		ВТ_СобытияЗадачПолные КАК ВТ_СобытияЗадачПолные
	               |	ГДЕ
	               |		ВТ_СобытияЗадачПолные.Событие = ЗНАЧЕНИЕ(Перечисление.ВидыСобытийЗадач.ИзмененаВажность)
	               |		И ВТ_СобытияЗадачПолные.ДатаСобытия МЕЖДУ &ДатаВремяОт И &ДатаВремяПо
	               |	
	               |	ОБЪЕДИНИТЬ ВСЕ
	               |	
	               ...
	               |		"""",
	               |		"""",
	               |		""#"" + ""Перенаправлена: "" + ПОДСТРОКА(ВТ_СобытияЗадачПолные.Комментарий, 1, 150) + "" "",
	               |		"""",
	               |		"""",
	               |		""""
	               |	ИЗ
	               |		ВТ_СобытияЗадачПолные КАК ВТ_СобытияЗадачПолные
	               |	ГДЕ
	               |		ВТ_СобытияЗадачПолные.Событие = ЗНАЧЕНИЕ(Перечисление.ВидыСобытийЗадач.Перенаправлена)
	               |		И ВТ_СобытияЗадачПолные.ДатаСобытия МЕЖДУ &ДатаВремяОт И &ДатаВремяПо
	               |	
	               |	ОБЪЕДИНИТЬ ВСЕ
	               |	
	               ...
	               |		"""",
	               |		"""",
	               |		""#"" + ""Изменен срок: "" + ПОДСТРОКА(ВТ_СобытияЗадачПолные.Комментарий, 1, 150) + "" "",
	               |		"""",
	               |		""""
	               |	ИЗ
	               |		ВТ_СобытияЗадачПолные КАК ВТ_СобытияЗадачПолные
	               |	ГДЕ
	               |		ВТ_СобытияЗадачПолные.Событие = ЗНАЧЕНИЕ(Перечисление.ВидыСобытийЗадач.ИзмененСрок)
	               |		И ВТ_СобытияЗадачПолные.ДатаСобытия МЕЖДУ &ДатаВремяОт И &ДатаВремяПо
	               |	
	               |	ОБЪЕДИНИТЬ ВСЕ
	               |	
	               ...
	               |		"""",
	               |		"""",
	               |		""#"" + ПОДСТРОКА(ВТ_СобытияЗадачПолные.Комментарий, 1, 150) + "" "",
	               |		""""
	               |	ИЗ
	               |		ВТ_СобытияЗадачПолные КАК ВТ_СобытияЗадачПолные
	               |	ГДЕ
	               |		ВТ_СобытияЗадачПолные.Событие = ЗНАЧЕНИЕ(Перечисление.ВидыСобытийЗадач.ВозвратКИсполнению)
	               |		И ВТ_СобытияЗадачПолные.ДатаСобытия МЕЖДУ &ДатаВремяОт И &ДатаВремяПо
	               |	
	               |	ОБЪЕДИНИТЬ ВСЕ
	               |	
	               ...
	               |		ВТ_СобытияЗадачПолные КАК ВТ_СобытияЗадачПолные
	               |	ГДЕ
	               |		ВТ_СобытияЗадачПолные.Событие = ЗНАЧЕНИЕ(Перечисление.ВидыСобытийЗадач.ИзмененоСостояниеДругое)
	               |		И ВТ_СобытияЗадачПолные.ДатаСобытия МЕЖДУ &ДатаВремяОт И &ДатаВремяПо) КАК ВложенныйЗапрос
	               |
	               |СГРУППИРОВАТЬ ПО
	               ...
	               |	ЗадачаИсполнителя.СрокИсполнения КАК СрокИсполнения,
	               |	ЗадачаИсполнителя.Инициатор КАК Автор,
	               |	ВЫБОР
	               |		КОГДА ЗадачаИсполнителя.ДатаНачала МЕЖДУ &ДатаВремяОт И &ДатаВремяПо
	               |			ТОГДА ИСТИНА
	               |		ИНАЧЕ ЛОЖЬ
	               |	КОНЕЦ КАК ЭтоНоваяЗадача,
	               |	ПОДСТРОКА(ЗадачаИсполнителя.Описание, 1, 250) КАК Описание,
	               |	NULL КАК ОсновнойОбъектАдресации,
	               |	NULL КАК ДополнительныйОбъектАдресации,
	               |	ЕСТЬNULL(ИсторийСобытийИзменений.ПользовательРедактировавшийЗадачу, ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)) КАК ПользовательРедактировавшийЗадачу,
	               |	ВЫБОР
	               |		КОГДА ИсторийСобытийИзменений.СобытиеОписание ЕСТЬ NULL
	               |			ТОГДА """"
	               |		КОГДА ЗадачаИсполнителя.ДатаНачала МЕЖДУ &ДатаВремяОт И &ДатаВремяПо
	               |			ТОГДА """"
	               |		ИНАЧЕ ИсторийСобытийИзменений.СобытиеОписание + ИсторийСобытийИзменений.СобытиеВажностьИПрочее + ИсторийСобытийИзменений.СобытиеПеренаправление + ИсторийСобытийИзменений.СобытиеСрок + ИсторийСобытийИзменений.СобытиеВозврат + ИсторийСобытийИзменений.СобытиеПрочиеИзменения
	               |	КОНЕЦ КАК СобытияРедактированнойЗадачи
	               |ИЗ
	               |	Задача.ЗадачиПользователя КАК ЗадачаИсполнителя
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИсполнителиЗадач КАК ИсполнителиЗадач
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ИсторийСобытийИзменений КАК ИсторийСобытийИзменений
	               |		ПО (ИсторийСобытийИзменений.Задача = ЗадачаИсполнителя.Ссылка)
	               |ГДЕ
	               |	ЗадачаИсполнителя.ПометкаУдаления = ЛОЖЬ
	               ...
	               |	И ЗадачаИсполнителя.Выполнена = ЛОЖЬ
	               |	И ВЫБОР
	               |			КОГДА НЕ(ЗадачаИсполнителя.ДатаНачала МЕЖДУ &ДатаВремяОт И &ДатаВремяПо)
	               |					И ИсторийСобытийИзменений.ПользовательРедактировавшийЗадачу ЕСТЬ НЕ NULL 
	               |					И ИсторийСобытийИзменений.ПользовательРедактировавшийЗадачу <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	               |				ТОГДА ИсторийСобытийИзменений.ПользовательРедактировавшийЗадачу <> ЗадачаИсполнителя.Исполнитель
	               |			ИНАЧЕ ЗадачаИсполнителя.Инициатор <> ЗадачаИсполнителя.Исполнитель
	               |					И (ЗадачаИсполнителя.ДатаНачала МЕЖДУ &ДатаВремяОт И &ДатаВремяПо)
	               |		КОНЕЦ
	               |	И ЗадачаИсполнителя.УзелОбмена = &УзелОбмена
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Исполнитель,
	               |	СрокИсполнения УБЫВ
		...
		СписокСобытий.Добавить(Перечисления.ВидыСобытийЗадач.ДополнениеОписания);
		СписокСобытий.Добавить(Перечисления.ВидыСобытийЗадач.ИзмененоСостояниеДругое);
		СписокСобытий.Добавить(Перечисления.ВидыСобытийЗадач.ИзмененаВажность);
		СписокСобытий.Добавить(Перечисления.ВидыСобытийЗадач.Перенаправлена);
		СписокСобытий.Добавить(Перечисления.ВидыСобытийЗадач.ИзмененСрок);
		СписокСобытий.Добавить(Перечисления.ВидыСобытийЗадач.ВозвратКИсполнению);
		Запрос.Параметры.Вставить("СписокСобытий", СписокСобытий);
		
		Запрос.Параметры.Вставить("УзелОбмена", ОбщегоНазначенияКупоросСервер.ЭтотУзелОбменаДляКупорос());
	...
	// { *Олег Каратаев [03.06.2023]
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "Задача.ЗадачаИсполнителя КАК", "Задача.ЗадачиПользователя КАК");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЗадачаИсполнителя.Автор КАК", "ЗадачаИсполнителя.Инициатор КАК");
	// }
...
Функция ОтправитьУведомлениеОНовыхЗадачах(Исполнитель, ЗадачиПоИсполнителю, АдресаПолучателей)
	...
	Для Каждого Задача Из ЗадачиПоИсполнителю.Строки Цикл
		// { *Олег Каратаев [21.11.2013]
		ТекстПисьма = ТекстПисьма + СформироватьПредставлениеЗадачиПользователя(Задача) + "<br><hr><br><br>";
		// --1С--
...
Функция СформироватьПредставлениеЗадачиПользователя(ЗадачаСтруктура)
	
	ШаблонСтроки = 
		НСтр("ru = '<b>Задача: %1</b><br><br>
		|
		|Крайний срок: %2<br>'");
		
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Ссылка.Важность) Тогда
		ШаблонСтроки = ШаблонСтроки + СтрШаблон(НСтр("ru = 'Важность: %1<br>'"), РаботаСЗадачами.ОпределитьВажностьHTML(ЗадачаСтруктура.Ссылка.Важность));
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Ссылка.Проект) Тогда
		ШаблонСтроки = ШаблонСтроки + НСтр("ru = 'Проект: %9<br>'");
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Исполнитель) Тогда
		ШаблонСтроки = ШаблонСтроки + НСтр("ru = 'Исполнитель: %3<br>'");
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.РольИсполнителя) Тогда
		ШаблонСтроки = ШаблонСтроки + НСтр("ru = 'Роль: %4<br>'");
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ОсновнойОбъектАдресации) Тогда
		ШаблонСтроки = ШаблонСтроки + НСтр("ru = 'Основной объект адресации: %5<br>'");
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ДополнительныйОбъектАдресации) Тогда
		ШаблонСтроки = ШаблонСтроки + НСтр("ru = 'Доп. объект адресации: %6<br>'");
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Описание) Тогда
		ШаблонСтроки = ШаблонСтроки + НСтр("ru = '<br>%8'");
	КонецЕсли;
	ШаблонСтроки = ШаблонСтроки + "<br>";
	
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ПользовательРедактировавшийЗадачу) Тогда
		ТекстДляВставки = СтрШаблон(НСтр("ru = '<br><font color=gray size=2>Изменение задачи: %1, %2. СОБЫТИЯ: %3</font><br>'"), ЗадачаСтруктура.ПользовательРедактировавшийЗадачу, ЗадачаСтруктура.Дата, ЗадачаСтруктура.СобытияРедактированнойЗадачи);
		ШаблонСтроки = ШаблонСтроки + ТекстДляВставки;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Автор) Тогда
		ШаблонСтроки = ШаблонСтроки + "%7";
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Ссылка.Основание) Тогда
		ШаблонСтроки = ШаблонСтроки + РаботаСЗадачами.СформироватьHTMLБлок("Основание", ЗадачаСтруктура.Ссылка.Основание);
	КонецЕсли;
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтроки,
		...
		РаботаСЗадачами.СформироватьHTMLБлок("Автор",ЗадачаСтруктура.Автор),
		РаботаСЗадачами.СформироватьHTMLБлок("Описание",ЗадачаСтруктура.Ссылка.Описание),
		ЗадачаСтруктура.Ссылка.Проект
		);
	
КонецФункции

Процедура УведомлениеСлужбыТехподдержкиОНовыхЗадачах() Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ДатаПоследнегоУведомления = Константы.ДатаУведомленияСлужбыПоддержкиОНовыхЗадачах.Получить();
	УстановитьПривилегированныйРежим(Ложь);
	...
	ЗаписьЖурналаРегистрации(БизнесПроцессыИЗадачиСервер.СобытиеЖурналаРегистрации(),
		УровеньЖурналаРегистрации.Информация, , ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Начато регламентное уведомление о новых задачах службы техподдержки за период %1 - %2.'"),
			ДатаПоследнегоУведомления, ДатаУведомления));
	...
	Для Каждого СтрокаИсполнителя Из ЗадачиПоИсполнителям.Строки Цикл
		Если Не СтрокаИсполнителя.Исполнитель.СлужбаТехподдержки Тогда
			Продолжить;	
		КонецЕсли;
		ОтправитьУведомлениеОНовыхЗадачах(СтрокаИсполнителя.Исполнитель, СтрокаИсполнителя, АдресаПолучателей);
	КонецЦикла;
	...
	УстановитьПривилегированныйРежим(Истина);
	Константы.ДатаУведомленияСлужбыПоддержкиОНовыхЗадачах.Установить(ДатаУведомления);
	УстановитьПривилегированныйРежим(Ложь);
	
	ЗаписьЖурналаРегистрации(БизнесПроцессыИЗадачиСервер.СобытиеЖурналаРегистрации(),
		УровеньЖурналаРегистрации.Информация, , ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Завершено регламентное уведомление о новых задачах службы техподдержки (уведомлено исполнителей: %1).'"),
			ЗадачиПоИсполнителям.Строки.Количество()));
