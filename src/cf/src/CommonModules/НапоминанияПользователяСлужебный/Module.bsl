...
//  * [Процедура]ПодключитьПроизвольноеНапоминание - вставка доп. параметра ДопПараметрыНапоминания для расширения заполняемости регистра #УведомлениеПоТелеграмВНапоминаниях #УведомлениеПоЭлектроннойПочтеВНапоминаниях
// 
// ТЕГИ ИЗМЕНЕНИЙ
//
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
//
Функция ПодключитьПроизвольноеНапоминание(Текст, ВремяСобытия, ИнтервалДоСобытия = 0, Предмет = Неопределено, Идентификатор = Неопределено,
	ДопПараметрыНапоминания = Неопределено) Экспорт  // { *Олег Каратаев [2021-12-26] Возможность вставки доп.параметров }
	
	// { *Олег Каратаев [26.12.2021]
	Если  ТипЗнч(ДопПараметрыНапоминания) = Тип("Структура") Тогда
		ПараметрыНапоминания = ДопПараметрыНапоминания;
	Иначе
