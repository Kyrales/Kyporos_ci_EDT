...
//  * [Процедура]ПринятьЗадачиКИсполнению - обязательная установка исполнителем текущего пользователя #TaskUsers #СостояниеЗадачи
//  * [Процедура]ОтменитьПринятиеЗадачКИсполнению - #СостояниеЗадачи
// 
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
Процедура ПринятьЗадачиКИсполнению(Задачи) Экспорт
	...
	Попытка
		...
		Для каждого Задача Из Задачи Цикл
			...
			// { *Олег Каратаев [24.02.2018]
			// обновление текущего состояния в регистре
			РегистрыСведений.ТекущиеСостоянияЗадач.ОбновитьТекущееСостояниеЗадачи(Задача,Справочники.СостоянияЗадач.ВРаботе);
			// }
...
Процедура ОтменитьПринятиеЗадачКИсполнению(Задачи) Экспорт
	...
	Попытка
		...
		Для каждого Задача Из Задачи Цикл
			...
			// { *Олег Каратаев [24.02.2018]
			// обновление текущего состояния в регистре
			РегистрыСведений.ТекущиеСостоянияЗадач.ОбновитьТекущееСостояниеЗадачи(Задача,Справочники.СостоянияЗадач.Зарегистрирована);
			// }
