...
//  * [Процедура]ПередФормированиемСпискаПолучателейРассылки - Получение всех пользователей из справочника "Пользователи","Проекты"
// 
// ТЕГИ ИЗМЕНЕНИЙ
//
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
Процедура ПередФормированиемСпискаПолучателейРассылки(ПараметрыПолучателей, Запрос, СтандартнаяОбработка, Результат) Экспорт
	...
	// { *Олег Каратаев [21.01.2018]
	Если ПараметрыПолучателей.ТипПолучателейРассылки.ПолноеИмя = "Справочник.Организации" Тогда
		
		// Переопределение запроса и параметра. Получение всех сотрудников организации из справочника "Пользователи".
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ИЗ
		               |	Справочник.Организации КАК СпрПолучателей
		               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Пользователи.КонтактнаяИнформация КАК тчКонтактнаяИнформация
		               |		ПО (тчКонтактнаяИнформация.Ссылка.Организация = СпрПолучателей.Ссылка)
		               |			И (тчКонтактнаяИнформация.Вид = &ВидКИ)
		               |ГДЕ
		               |	СпрПолучателей.Ссылка В
		               |			(ВЫБРАТЬ
		               ...
		               |	И НЕ СпрПолучателей.Ссылка В
		               |				(ВЫБРАТЬ
		               |					ИсключенияПолучателей.Получатель КАК Получатель
		               |				ИЗ
	...
	ИначеЕсли ПараметрыПолучателей.ТипПолучателейРассылки.ПолноеИмя = "Справочник.Проекты" Тогда
		
		// Переопределение запроса и параметра. Получение всей рабочей группы из справочника "Проекты".
		Запрос.Текст = "ВЫБРАТЬ
		               |	ТаблицаПолучателей.Получатель КАК Получатель,
		               |	ТаблицаПолучателей.Исключен КАК Исключен
		               |ПОМЕСТИТЬ втТаблицаПолучателей
		               ...
		               |////////////////////////////////////////////////////////////////////////////////
		               |ВЫБРАТЬ
		               |	ТаблицаПользователейПроекта.Проект КАК Проект,
		               |	ТаблицаПользователейПроекта.EMail КАК EMail
		               |ПОМЕСТИТЬ втТаблицаПользователейПроекта
		               |ИЗ
		               |	&ТаблицаПользователейПроекта КАК ТаблицаПользователейПроекта
		               |;
		               |
		               |////////////////////////////////////////////////////////////////////////////////
		               |ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		               |	СпрПолучателей.Ссылка КАК Получатель,
		               |	втТаблицаПользователейПроекта.EMail КАК EMail
		               |ИЗ
		               |	Справочник.Проекты КАК СпрПолучателей
		               |		ЛЕВОЕ СОЕДИНЕНИЕ втТаблицаПользователейПроекта КАК втТаблицаПользователейПроекта
		               |		ПО (втТаблицаПользователейПроекта.Проект = СпрПолучателей.Ссылка)
		               |ГДЕ
		               |	СпрПолучателей.Ссылка В
		               |			(ВЫБРАТЬ
		               |				Получатели.Получатель КАК Получатель
		...
		// Формирование таблицы пользователей проекта из функции Справочники.Проекты.МассивПользователейПроекта().
		ТаблицаПользователейПроекта = Новый ТаблицаЗначений;
		ТаблицаПользователейПроекта.Колонки.Добавить("Проект", Новый ОписаниеТипов("СправочникСсылка.Проекты"));
		ТаблицаПользователейПроекта.Колонки.Добавить("EMail", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(100)));
		
		Для каждого ТекСТрокаПолучателя Из ПараметрыПолучателей.Получатели Цикл
			
			// добавление пользователей проекта
			МассивПользователейПроекта = Справочники.Проекты.МассивПользователейПроекта(ТекСТрокаПолучателя.Получатель, Истина);
			Для каждого ТекСтрокаПользователя Из МассивПользователейПроекта Цикл
				НовСтрокаПользователяПроекта = ТаблицаПользователейПроекта.Добавить();
				НовСтрокаПользователяПроекта.Проект = ТекСТрокаПолучателя.Получатель;
				НовСтрокаПользователяПроекта.EMail = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформацииОбъекта(
														ТекСтрокаПользователя, 
														Справочники.ВидыКонтактнойИнформации.EmailПользователя);
			КонецЦикла;
			
			// добавление контактной информации проекта
			ВидКонтактнойИнформации = УправлениеКонтактнойИнформацией.ВидКонтактнойИнформацииПоИмени("EmailПроекта");
			EmailПроекта = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформацииОбъекта(ТекСТрокаПолучателя.Получатель, ВидКонтактнойИнформации);
			Если ЗначениеЗаполнено(EmailПроекта) Тогда
				НовСтрокаПользователяПроекта = ТаблицаПользователейПроекта.Добавить();
				НовСтрокаПользователяПроекта.Проект = ТекСТрокаПолучателя.Получатель;
	            НовСтрокаПользователяПроекта.EMail = EmailПроекта;
			КонецЕсли;
			
		КонецЦикла;
		Запрос.УстановитьПараметр("ТаблицаПользователейПроекта", ТаблицаПользователейПроекта);
