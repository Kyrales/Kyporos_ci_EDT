...
Функция СформироватьКодДиаграммыБизнесДоменов(СсылкаНаДокумент) Экспорт
	...
	Для Каждого ТекСистема Из СистемыСДоменами Цикл
		...
		ОбщегоНазначенияКупоросКлиентСервер.ПрисоединитьСтроку(ИсходныйКод, КодПоСистеме, Символы.ПС);
		
		Если ТекСистема.Значение.ЭтоОсновнаяСистема Тогда
			ИдентификаторОсновнойСистемы = "sys" + ОбщегоНазначенияКупоросКлиентСервер.ИдентификаторДляФормул(ТекСистема.Ключ.Наименование);
		КонецЕсли;
...
Функция ЗаполнитьПараметрыИСДляДиаграммы(ИнформационнаяСистема, ДополнительныеПараметры = Неопределено)
	...
	Параметры.Параметр1 = ОбщегоНазначенияКупоросКлиентСервер.ИдентификаторДляФормул(НаименованиеДляВывода); // Переменная для C4. Ранее: СсылкаНаОбъект.Код;
...
Функция ЗаполнениеИсходногоКодаБлоков(Контейнеры, ПараметрыДляИсходногоКодаБлока)
	...
	Для Каждого ТекКонтейнер Из Контейнеры Цикл
		...
		Если ЕстьВнешнийСервисИсточникИлиПриемника
			...
			ОбщегоНазначенияКупоросКлиентСервер.ПрисоединитьСтроку(ИсходныйКодБлоковВнешнихСервисов, ПромежуточныйКодБлоков, Символы.ПС);
		Иначе
			ОбщегоНазначенияКупоросКлиентСервер.ПрисоединитьСтроку(ИсходныйКодБлоков, ПромежуточныйКодБлоков, Символы.ПС);
		КонецЕсли;
...
Функция ЗаполнениеИсходногоКодаСвязейБлоков(Контейнеры, МассивИнтеграций, БазовыйКод)
	...
	Для Каждого ТекПараметрыИнтеграции Из МассивИнтеграций Цикл
		...
		ОбщегоНазначенияКупоросКлиентСервер.ПрисоединитьСтроку(СтрокиТеговСвязей, СтрокаТегаСвязи, Символы.ПС);
		...
		Для Каждого ТекТочкаИнтеграции Из СписокТочекИнтеграции Цикл
			...
			Если ОбщегоНазначенияКупоросСервер.ПроверитьДублированиеСтрокТаблицы(ТаблицаДляПроверкиДублей, КолонкиДляПроверкиНаДубли, ДублированныеСтроки) Тогда
				...
				Иначе
					ОбщегоНазначенияКупоросКлиентСервер.ПрисоединитьСтроку(ДублированныеСтроки[0].ИндексСтрока, СтрокаИндексаНовая, "");
...
Функция ЗаполнениеИсходногоКодаБлокаПоТочкеИнтеграции(Контейнеры, ТекТочкаИнтеграции, КолВоИнтеграций, НомерИнтеграции,
	...
	Если СтрНайти(ИсходныйКодСоединений, ПромежуточныйКодСоединений) = 0 Тогда
		...
		ОбщегоНазначенияКупоросКлиентСервер.ПрисоединитьСтроку(ИсходныйКодСоединений, ПромежуточныйКодСоединений, Символы.ПС);
...
Функция ЗаполнениеБлокаСистемыСКонтейнерами(ТекКонтейнер, Контейнеры)
	...
	Иначе
		...
		Для Каждого ТекРесурс Из ИспользуемыеСистемойРесурсы Цикл
			...
			ОбщегоНазначенияКупоросКлиентСервер.ПрисоединитьСтроку(КодКонтейнеровВнутриСистемы, КодКонтейнераРесурса, Символы.ПС);
		КонецЦикла;
	...
	Для Каждого ТекРесурс Из ИспользуемыеСистемойРесурсы Цикл
		...
		ОбщегоНазначенияКупоросКлиентСервер.ПрисоединитьСтроку(КодБлокаДляДиаграммы, КодСвязиРесурсаИСистемы, Символы.ПС);
...
Функция ЗаполнениеБлокаСистемыСБизнесДоменами(ИнформационнаяСистема, ДанныеПоСистеме)
	...
	Для Каждого ТекСтрока Из ДанныеПоСистеме.ТаблицаДоменов Цикл
		...
		ОбщегоНазначенияКупоросКлиентСервер.ПрисоединитьСтроку(ПромежуточныйКодДомена, КодБлокаДляДиаграммы, Символы.ПС);
	...
	Иначе	
		КодДиаграммыПоСистеме = НСтр("ru = 'System_Boundary(sys%1, ""%2"") {
                                      |   %3
                                      |}'");
		НаименованиеСистемыДляВывода = ОбщегоНазначенияКупоросКлиентСервер.ИдентификаторДляФормул(ИнформационнаяСистема);
...
Функция ЗаполнениеКодаСвязейДоменов(ДокументАрхитектурноеРешение)
	...
	Для каждого ТекСтрока Из ДокументАрхитектурноеРешение.СвязиДоменов Цикл
		...
		ПромежуточныйКодСоединений = СтрШаблон(НСтр("ru = '
                                                     |Rel(%1, %2, ""%3"",""%4"",""%5""%6)'"),
			ОбщегоНазначенияКупоросКлиентСервер.ИдентификаторДляФормул(ТекСтрока.Источник.Наименование),  // 1
			ОбщегоНазначенияКупоросКлиентСервер.ИдентификаторДляФормул(ТекСтрока.Приемник.Наименование),  // 2
			ОписаниеПотока,                                                                     // 3
		...
		ОбщегоНазначенияКупоросКлиентСервер.ПрисоединитьСтроку(КодСвязей, ПромежуточныйКодСоединений, Символы.ПС);
