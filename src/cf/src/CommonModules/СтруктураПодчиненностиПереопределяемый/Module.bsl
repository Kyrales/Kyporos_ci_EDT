...
//  * [Процедура]ПриПолученииПредставления - расшифровка для Запросов из внешних сервис-десков
// 
// ТЕГИ ИЗМЕНЕНИЙ
//
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
Процедура ПриПолученииПредставления(ТипДанных, Данные, Представление, СтандартнаяОбработка) Экспорт
	  ...
	  // { *Олег Каратаев [23.04.2020]
	  Если ПолучитьФункциональнуюОпцию("ИспользоватьПланированиеЗадач")
		   И ТипДанных = Тип("ДокументСсылка.ПриоритизацияЗадач")
		   И ЗначениеЗаполнено(Данные.ДополнительныйРеквизит1)
		   И Тип("ЗадачаСсылка.ЗадачиПользователя") = ТипЗнч(Данные.ДополнительныйРеквизит1) Тогда
		  
		   // Выводим информацию о порядке приоритетов.
	  	   РеквизитыЗапроса = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Данные.Ссылка, "Номер, Дата, СписокЗадач");
		   СписокЗадач = РеквизитыЗапроса.СписокЗадач.Выгрузить();
		   
		   // поиск строки
		   СтрокаЗадачи = СписокЗадач.Найти(Данные.ДополнительныйРеквизит1, "Задача");
		   
		   // План - в случае если задача распределена на текущий период выполнения по документу Приоритизации задач.
		   ЗадачаРаспределена = ?(СтрокаЗадачи.Распределена, НСтр("ru = ' - План'"), "");
		   
		   Представление = СтрШаблон(НСтр("ru = 'Приоритизация %1 от %2 - [%3]%4'"), РеквизитыЗапроса.Номер, Формат(РеквизитыЗапроса.Дата, "ДФ='MMMM yyyy'"), СтрокаЗадачи.НомерСтроки, ЗадачаРаспределена);
