...
//  * [Функция]ЗначениеСвойстваОбъекта - исправление ошибки в случае попытки обработки битой ссылки
// 
// ТЕГИ ИЗМЕНЕНИЙ
//
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
Функция ЗначениеСвойстваОбъекта(Объект, СтрокаСвойствОбъекта)
	...
	// { *Олег Каратаев [04.02.2018]
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Значение)) 
		И Не ОбщегоНазначения.СсылкаСуществует(Значение) Тогда
		Возврат Неопределено;
	КонецЕсли;
	// }
