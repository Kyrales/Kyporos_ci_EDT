...
//  * [Процедура]ПриДобавленииСерверныхОповещений - получение кол-ва задач через серверные оповещения
// 
// ТЕГИ ИЗМЕНЕНИЙ
//
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
Процедура ПриДобавленииСерверныхОповещений(Оповещения) Экспорт
	...
	// { *Олег Каратаев [17.07.2022]
	// TODO: Переделать на механизм УведомлениеКлиента в 8.3.26
	// Функционал ниже приводит к ошибке в клиент-серверном режиме:
	//	Процедура "РаботаСЗадачами.ПриОтправкеСерверногоОповещения" не выполнена по причине:Метод объекта не обнаружен (Количество)
	//Если ПолучитьФункциональнуюОпцию("ИспользоватьОценкуЗадач") Тогда
	//	
	//	Оповещение = СерверныеОповещения.НовоеСерверноеОповещение(
	//	"УчетЗадачПользователей.ПроверитьЗадачиБезОценки");
	//	Оповещение.ИмяМодуляОтправки  = "РаботаСЗадачами";
	//	Оповещение.ИмяМодуляПолучения = "РаботаСЗадачамиКлиент";
	//	Оповещение.ПериодПроверки = 600;
	//	
	//	Оповещения.Вставить(Оповещение.Имя, Оповещение);	
	//	
	//КонецЕсли;
