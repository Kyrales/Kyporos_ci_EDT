...
// 
// ФОРМА ИЗМЕНЯЛАСЬ
//  * [Реквизиты]ОсновнойОбъект.Тип - +ЗадачаСсылка  #SWKY-000051
//  * [Реквизиты]ДеревоПодчиненныеОбъекты.Ссылка.Тип - +ЗадачаСсылка  #SWKY-000051
//  * [Реквизиты]ДеревоРодительскиеОбъекты.Ссылка.Тип - +ЗадачаСсылка   #SWKY-000051
//  * [Элементы]ТаблицаОтчета - заполнено событие "Выбор" процедурой ТаблицаОтчетаВыбор SWKY-000097
//
// НОВЫЕ/ИЗМЕНЕННЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ
//  * [Процедура]МетаданныеТипаРеквизита - #SWKY-000051
//  * [Процедура]ЗапросПоРеквизитамОбъектов - Добавление родителя в ДополнительныйРеквизит1 для дальнейшего использования (при необходимости)
//  + [Процедура]ТаблицаОтчетаВыбор - SWKY-000097
//
// ТЕГИ ИЗМЕНЕНИЙ
//  #SWKY-000051
//   Исправление ошибки 1С: возможность вывода данных объекту метаданных из Задачи
//
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<

// Разделитель описания изменений модуля формы >> *Олег Каратаев
...
Функция ЗапросПоРеквизитамОбъектов(СписокОбъектов)
	...
	Для каждого ТипОбъекта Из ОбъектыПоТипу Цикл
		...
		// { *Олег Каратаев [08.01.2021]
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "NULL Как ДополнительныйРеквизит1,", "&Родитель Как ДополнительныйРеквизит1,");
		Запрос.УстановитьПараметр("Родитель", ТекущийОбъект);
		// }
...
Функция МетаданныеТипаРеквизита(ТипРеквизита)
	...
	Если Не Результат.ЭтоДокумент
		...
		И Не Метаданные.Задачи.Содержит(МетаданныеРеквизита) // { *Олег Каратаев [2020-03-09]  SWKY-000051}
		Тогда
		Возврат Результат;
	КонецЕсли;
...
// { *Олег Каратаев [10.03.2021]

&НаКлиенте
Процедура ТаблицаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	
	Если ТипЗнч(Область.Расшифровка) = Тип("ДокументСсылка.ПриоритизацияЗадач") Тогда
	    // Пример: Приоритизация SWKY-000002 от Февраль 2021 - [2] - План.
		// Необходимо определить значение в квадратных скобках [%]. 
		// Это число будет являться номером строки. После открытия документа и спозиционироваться на элементе этой строки.
		СтандартнаяОбработка = Ложь;
				
		ТекстВывода = Область.Текст;  
		
		НайденнаяПозицияНачало = СтрНайти(ТекстВывода, " - [");
		НайденнаяПозицияКонец = СтрНайти(ТекстВывода, "]",, НайденнаяПозицияНачало);
		
		ЧислоСимволов = НайденнаяПозицияКонец - НайденнаяПозицияНачало - 4;
		НомерСтрокиДляОбработки = Формат(Сред(ТекстВывода, НайденнаяПозицияНачало + 4, ЧислоСимволов), "ЧГ=0");
		НомерСтрокиЧисло = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(НомерСтрокиДляОбработки);
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Ключ", Область.Расшифровка);
		ПараметрыФормы.Вставить("НомерСтрокиДляПозиционирования", НомерСтрокиЧисло);
		
		ОткрытьФорму("Документ.ПриоритизацияЗадач.Форма.ФормаДокумента", ПараметрыФормы, ЭтаФорма);
