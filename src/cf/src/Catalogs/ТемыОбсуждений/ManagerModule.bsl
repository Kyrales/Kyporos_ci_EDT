...
Функция ПечатьКарточки(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		Если СведенияОТеме <> Неопределено Тогда
			...
			ОбластьПоследнееСообщение.Параметры.ТекстПоследнегоСообщения = ТекстПоследнегоСообщенияТемы(ОбъектПечати);
...
Функция ТекстПоследнегоСообщенияТемы(Тема)
	...
	ЗапросПоследнееСообщение.УстановитьПараметр("Тема", Тема);
	Выборка = ЗапросПоследнееСообщение.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ТекстПоследнегоСообщения = Выборка.ТекстСообщения;
	Иначе
		ТекстПоследнегоСообщения = "";
	КонецЕсли;
	
	Возврат ТекстПоследнегоСообщения;

КонецФункции // ТекстПоследнегоСообщения()
