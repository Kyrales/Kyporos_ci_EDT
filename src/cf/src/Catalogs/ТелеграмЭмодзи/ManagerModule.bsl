...
Процедура ЗагрузитьСлепок(ИмяСлепка) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СправочникОбъектЭмодзи = Выборка.Ссылка.ПолучитьОбъект();
		СправочникОбъектЭмодзи.Удалить();
	...
	Для Каждого Структура Из Массив Цикл
		...
		Если Структура.ЭтоГруппа Тогда
			...
			СправочникОбъектЭмодзи = Справочники.ТелеграмЭмодзи.СоздатьГруппу();
			ЗаполнитьЗначенияСвойств(СправочникОбъектЭмодзи, Структура);
			СправочникОбъектЭмодзи.Записать();
			
		Иначе
			
			СправочникОбъектЭмодзи = Справочники.ТелеграмЭмодзи.СоздатьЭлемент();
			ЗаполнитьЗначенияСвойств(СправочникОбъектЭмодзи, Структура, , "Картинка");
			СправочникОбъектЭмодзи.Картинка = Новый ХранилищеЗначения(Base64Значение(Структура.Картинка));
			СправочникОбъектЭмодзи.Родитель = Справочники.ТелеграмЭмодзи.НайтиПоНаименованию(Структура.РодительНаименование, Истина, Справочники.ТелеграмЭмодзи.ПустаяСсылка());
			СправочникОбъектЭмодзи.Записать();
