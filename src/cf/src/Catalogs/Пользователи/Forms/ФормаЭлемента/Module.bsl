...
//  * [Процедура]ПриСозданииНаСервере - ГруппаДопИнформации: Организация, ПредставлениеВДокументах, СлужбаТехподдержки, ТехУведомленияНаЭлектроннуюПочту 
//
//}} *Олег Каратаев <<<<<<<<<<<<<<<<<<<<<<< ОПИСАНИЕ ИЗМЕНЕНИЙ <<<<<<<<<<<<<<<<<<<<<<<<<<<

// Разделитель описания изменений модуля формы >> *Олег Каратаев
Перем ИзмененияМодуляФормы;  
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// { *Олег Каратаев [22.07.2021]
	// Добавить группу.
	ГруппаДопИнформации = Элементы.Добавить(
		"ГруппаДопИнформации",
		Тип("ГруппаФормы"),
		Элементы.ГруппаДанныеШапки);
		
	ГруппаДопИнформации.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаДопИнформации.Отображение = ОтображениеОбычнойГруппы.СильноеВыделение;
	ГруппаДопИнформации.Заголовок = "Дополнительная информация";

	// Добавить поля ввода.
	ПолеВвода = Элементы.Добавить("Организация", Тип("ПолеФормы"), ГруппаДопИнформации);
	ПолеВвода.Вид = ВидПоляФормы.ПолеВвода;
	ПолеВвода.ПутьКДанным = "Объект.Организация";
	
	ПолеВвода = Элементы.Добавить("ПредставлениеВДокументах", Тип("ПолеФормы"), ГруппаДопИнформации);
	ПолеВвода.Вид = ВидПоляФормы.ПолеВвода;
	ПолеВвода.ПутьКДанным = "Объект.ПредставлениеВДокументах";
	
	ПолеВвода = Элементы.Добавить("СлужбаТехподдержки", Тип("ПолеФормы"), ГруппаДопИнформации);
	ПолеВвода.Вид = ВидПоляФормы.ПолеФлажка;
	ПолеВвода.ПутьКДанным = "Объект.СлужбаТехподдержки";
	ПолеВвода.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	ПолеВвода.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
	
	ПолеВвода = Элементы.Добавить("ТехУведомленияНаЭлектроннуюПочту", Тип("ПолеФормы"), ГруппаДопИнформации);
	ПолеВвода.Вид = ВидПоляФормы.ПолеФлажка;
	ПолеВвода.ПутьКДанным = "Объект.ТехУведомленияНаЭлектроннуюПочту";
	ПолеВвода.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	// }
