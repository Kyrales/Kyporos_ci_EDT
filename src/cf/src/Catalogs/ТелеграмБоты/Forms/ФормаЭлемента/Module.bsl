	...
	Если ЗначениеЗаполнено(Объект.ИдентификаторРегламентногоЗадания) Тогда
		...
		Если РегламентноеЗадание <> Неопределено Тогда
			...
			Исключение
				...
				ИмяСобытия = НСтр("ru = 'Телеграм.ПолучениеОбновленияТелеграм'", ОбщегоНазначения.КодОсновногоЯзыка());
				ТекстКомментария = НСтр("ru = 'При немедленном получении сообщения Телеграм возникает ошибка: 
				          |%1'", ОбщегоНазначения.КодОсновногоЯзыка()); 
				КомментарийДляЖурнала = СтрШаблон(ТекстКомментария, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура УдалитьВебхукВыбор(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		...
		СтрокаJSON = СтрокаJSONУдалитьВебхук();
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("СтрокаJSON", СтрокаJSON);
		ОткрытьФорму("Справочник.ТелеграмБоты.Форма.ФормаПоказаJSON", ПараметрыОткрытия);
...
Процедура ОбновитьИнформациюОПоследнемВыполненииСервер()
	...
	Если ЗначениеЗаполнено(Объект.ИдентификаторРегламентногоЗадания) Тогда
		...
		Если РегламентноеЗадание <> Неопределено Тогда
			...
			ВремяНачала = ТекущаяДатаСеанса() - 3600;
...
&НаСервере
Функция СтрокаJSONУдалитьВебхук()
...
КонецФункции // УдалитьВебхукНаСервере()
